...
- 支援帳號識別與對話歷史管理（MVP 階段為最簡明文密碼；權限、登入紀錄等安全強化留待 Phase 4）。
...
- 上傳 TXT、PDF、CS（C# 原始碼）作為知識庫資料（MVP 範圍；Phase 2 擴充更多格式）。
- 針對已上傳的文件進行問答。
- 查看回答引用的來源檔案與位置。
- 要求 AI 根據知識庫內容產生簡報大綱。
- 要求 AI 產生完整 PPTX 簡報。
...
管理員（在 MVP 階段以「擁有後端 shell 存取權的部署者」身份存在，無 admin web UI、無 API 層權限區分）可以：

- 透過 CLI 建立其他使用者帳號（直接寫 username + password 到 DB）。
- 第二階段才加入：共用 RAG 知識庫、登入紀錄查詢、系統使用紀錄、模型設定、權限管理等 web 介面。
...
- TXT（純文字檔，直接讀取）
- PDF（透過 pypdf 或同類庫做文字擷取）
- CS（C# 原始碼檔；以純文字讀取，不做語法樹分析，按字元/行 chunking）
...
- 音訊逐字稿
- 企業內部資料庫資料
- 程式碼語法樹感知 chunking（如 .cs 改用 Roslyn / tree-sitter 切方法層級）
...
系統需支援最簡帳號登入，**目的純粹是識別身份以隔離資料**，不追求安全強度。

### MVP 功能需求（最簡）

* 使用者需登入後才能使用系統。
* 帳號由 admin 透過 CLI 直接寫入 DB（username + password）。
* 密碼以明文存於 DB（MVP 階段為內部展示／開發用，明確接受此風險；production 部署需自行加固）。
* Login 時直接字串比對 username + password。
* 通過後回一個簡易 token（格式 `u_<user_id>`），前端在 `Authorization: Bearer u_<id>` header 帶上。
* Token 永不過期，server 端不維護 session 狀態，logout 純前端清掉本地 token。
* **不做**：login_logs / IP / User-Agent 紀錄、密碼雜湊、JWT、refresh token、rate limit、account 停用、admin 權限檢查、登入失敗 audit。
* 上述被砍項目改在第二階段（Phase 4 安全強化）統一處理。
...
### 4.5.3 Admin 共用資料（Phase 4，不在 MVP）

第二階段才加入：admin 共用 RAG 知識庫、共用文件上傳/刪除、被授權使用者清單管理、共用＋個人混合檢索。
...
* POST /auth/login — 驗證 username + password，回 `{ token: "u_<id>", user: {...} }`
* GET /auth/me — 解析 token，回當前使用者資料

第二階段才加入：POST /auth/logout、refresh、forgot password、admin user 管理 endpoints。
...
* 安全強化：密碼雜湊、JWT、refresh token、login_logs、rate limit、admin 權限檢查、登出 endpoint。
* Admin 共用知識庫。
* 多角色細緻權限。
* PPTX / DOCX / Markdown / CSV / XLSX / HTML / OCR 等 RAG 格式擴充。
* 程式碼語法樹感知 chunking。
* 進階簡報範本解析。
* 簡報線上編輯器。
...
* 最簡登入（明文密碼、token = `u_<id>`、無 logout endpoint）
* 基本聊天 UI
* LLM streaming
* 對話歷史
* Docker Compose 基礎環境
...
* 使用者管理（admin web UI）
* Admin 共用知識庫
* 安全強化：密碼雜湊、JWT、refresh token、token 撤銷、login_logs、IP/UA 紀錄、rate limit、API 層 admin 權限檢查、logout endpoint
* 權限控管
* 系統設定
